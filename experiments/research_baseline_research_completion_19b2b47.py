...
def _snapshot_pair(
    ...
    left = canonicalize_declared_state(reference_raw)
    right = canonicalize_declared_state(simulator_raw)
...
def _step(
    ...
    reference_state, simulator_state, state = _snapshot_pair(candidate, token)
...
def run_case(
    ...
    for action in case.actions:
        row["steps"].append(_step(candidate, token, action.as_dict(), meter))
        if on_progress is not None:
            on_progress(row)
